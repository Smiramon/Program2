package Program2;
//Program2: delivers images as GET requests
//WebWorker.java (updated)
//CS371
//Written by Sergio Baray
//Date written:02/09/18
   ...
   try {
      ...
      String path = readHTTPRequest(is);
  	  ...
  	  path = path.substring(1);
  	  														...
  	  														//distinguishing between GIF, JPEG PNG image files
  	  String contentType = "";
  	  														//Check GIF
  	  if(path.toLowerCase().contains((".GIF").toLowerCase()) == true){
  		contentType = "image/gif";
  	  														...
  	  														//Check JPEG
  	  else if(path.toLowerCase().contains((".JPG").toLowerCase()) == true){
  		...
  		contentType = "image/png";
  	  														...
  	  														//files are assumed to be text/html
  	  else{
		...
		if(f.exists() && !f.isDirectory()){
			...
			while ((strLine = br.readLine()) != null){
				...
				if(strLine.toLowerCase().contains(("<cs371date>").toLowerCase()) == true){
			        ...
			        os.write("Server's identification string : This is Sergio's server".getBytes());
				}
